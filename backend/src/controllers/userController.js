...
async function listUsers(req, res) {
  const result = await pool.query(
    `SELECT u.id, u.name, u.email, u.role, u.active, u.unit_id, un.name AS unit_name
...
async function createUser(req, res) {
  ...
  const result = await pool.query(
    `INSERT INTO users (name, email, password_hash, role, unit_id)
     VALUES ($1, $2, $3, $4, $5) RETURNING id, name, email, role, active, unit_id`,
    [name, email, hash, role, unit_id || null]
  );
...
async function updateUser(req, res) {
  ...
  values.push(id);
  const result = await pool.query(
    `UPDATE users SET ${fields.join(', ')} WHERE id = $${idx} RETURNING id, name, email, role, active, unit_id`,
    values
  );
